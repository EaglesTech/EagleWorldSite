...
class InvitesDriver implements IOTS_GuildAction
{
    ...
    public function listRequests()
    {
        ...
        foreach( $this->db->query('SELECT `player_id` FROM guild_invites WHERE `guild_id` = '.$this->guild->id) as $invite)
        {
    ...
    {
        $this->db->query('INSERT INTO guild_invites (`player_id`, `guild_id`) VALUES ('.$player->getId().', '.$this->guild->id.')');
    }

    // un-invites player
    public function deleteRequest(OTS_Player $player)
    {
        $this->db->query('DELETE FROM guild_invites WHERE `player_id` = '.$player->getId().' AND `guild_id` = '.$this->guild->id);
    }
