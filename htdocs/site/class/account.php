...
class Account extends SQL {
	...
	public function exists() {
		...
		if ($this->failed()) {
			throw new Exception('Account::exists() cannot determine whether account exists');
		}
		if ($this->num_rows() > 0) {
			return true;
		} else {
	...
	public function addRecoveryKey() {
		$this->attrs['recovery_key'] = strtoupper(substr(str_shuffle(md5(mt_rand()).md5(time())), 0, 32));
	...
	public function canVote($option) {
		$query = 'SELECT nicaw_polls.id FROM nicaw_polls, nicaw_poll_options WHERE nicaw_polls.id = nicaw_poll_options.poll_id
		...
		if ($this->num_rows() > 1) {
			throw new Exception('Unexpected SQL answer.');
		}
		$a = $this->fetch_array();
		$poll_id = $a['id'];
		$query = 'SELECT * FROM nicaw_poll_votes, nicaw_poll_options WHERE nicaw_poll_options.poll_id = '.$this->quote($poll_id).'
